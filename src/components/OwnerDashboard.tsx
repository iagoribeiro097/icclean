...
export default function OwnerDashboard({ bookings = [], onUpdateStatus, onClearAllBookings, onDeleteBooking, onDeleteClient, onBackToSite }: OwnerDashboardProps) {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    // Aceita a nova senha 'Is@@c123*' (e variações comuns de teclado 'Is@@c123' / 'Is@c123*') assim como a anterior
    const allowedPasswords = ['Is@@c123*', 'Is@@c123', 'Is@c123*', 'Lulu0987*'];
  ...
  const approvedCount = safeBookings.filter(b => b && b.status === 'approved').length;
  const rejectedCount = safeBookings.filter(b => b && b.status === 'rejected').length;
  ...
  safeBookings.forEach(b => {
    ...
    const key = phone || name || b.id || Math.random().toString();
  ...
  const approvedBookings = useMemo(() => {
    return safeBookings.filter(b => b && b.status === 'approved');
  }, [safeBookings]);
                  ...
                  >
                    ...
                    <div className="space-y-3 pt-3 border-t border-slate-800 text-xs text-slate-300">
                      ...
                      <div className="flex items-center gap-2.5">
                        ...
                        <span>
                          <strong className="text-slate-100">Data agendada:</strong> {b.date && typeof b.date === 'string' && b.date.includes('-') ? b.date.split('-').reverse().join('/') : (b.date || '')}
                        </span>
                            ...
                            {cell.bookings.map((bk, bIdx) => (
                              <div key={bIdx} className="text-[10px] text-emerald-200/90 truncate font-semibold flex items-center justify-between gap-1 bg-emerald-900/30 px-1.5 py-1 rounded border border-emerald-800/40">
                                <div className="flex items-center gap-1 truncate">
                                  ...
                                  <span className="truncate">{((bk.clientName || '').split(' ')[0] || 'Cliente')} ({bk.time || ''})</span>
                                </div>
                                <a
                ...
                <div>
                  <h4 className="text-lg font-bold text-slate-100 flex items-center gap-2">
                    ...
                    {selectedCalendarDate ? (
                      <>Higienizações Confirmadas em {selectedCalendarDate.includes('-') ? selectedCalendarDate.split('-').reverse().join('/') : selectedCalendarDate}</>
                    ) : (
